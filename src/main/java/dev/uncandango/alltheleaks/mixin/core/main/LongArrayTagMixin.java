...
public class LongArrayTagMixin implements Lockable {
	@Unique
   ...
   public abstract boolean addTag(int index, Tag tag);
	...
	@WrapMethod(method = {"set(ILnet/minecraft/nbt/LongTag;)Lnet/minecraft/nbt/LongTag;"})
	private LongTag alt$checkIfLockedSet(int p_128813_, LongTag p_128814_, Operation<LongTag> original){
		if (atl$locked) {
		...
		return original.call(p_128813_, p_128814_);
	}

	@WrapMethod(method = {"add(ILnet/minecraft/nbt/LongTag;)V"})
	private void alt$checkIfLockedAdd(int p_128832_, LongTag p_128833_, Operation<Void> original){
		if (atl$locked) {
			var error = new ATLUnsupportedOperation("An LongArrayTag was locked by mod AllTheLeaks, modifications of LongArrayTag from locked CompoundTags are not allowed!");
			var message = String.format(Locale.ROOT, "Cannot add entries on : longarraytag \"%s\".",this);
			AllTheLeaks.LOGGER.warn(message);
			throw error;
		}
		original.call(p_128832_, p_128833_);
	}

	@WrapMethod(method = {"remove(I)Lnet/minecraft/nbt/LongTag;"})
	private LongTag alt$checkIfLockedRemove(int p_128627_, Operation<LongTag> original){
		if (atl$locked) {
			var error = new ATLUnsupportedOperation("An LongArrayTag was locked by mod AllTheLeaks, modifications of LongArrayTag from locked CompoundTags are not allowed!");
			var message = String.format(Locale.ROOT, "Cannot remove entries on : longarraytag \"%s\".",this);
			AllTheLeaks.LOGGER.warn(message);
			throw error;
		}
